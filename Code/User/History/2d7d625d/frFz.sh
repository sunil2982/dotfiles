...
package_update() {
    ...
    if [ -n "$(command -v pacman)" ]; then
        # Use --hold or wait logic, or better yet, run kitty and hold it or capture completion. 
        # Alternatively, run kitty with a shell that waits, or pass the command correctly.
        kitty --title update sh -c "$upd_script; echo; echo 'Press Enter to close...'; read"
        exit_code=$?
